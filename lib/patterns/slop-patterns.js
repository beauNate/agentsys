...
function deepFreeze(obj) {
  ...
  // Using for-of loop instead of forEach avoids function call overhead
...
function isFileExcluded(filePath, excludePatterns) {
  ...
  // Create cache key from file path and pattern signature
  const patternsKey = excludePatterns.join('|');
  const cacheKey = `${filePath}::${patternsKey}`;
